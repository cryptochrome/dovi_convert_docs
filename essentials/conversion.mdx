...
This scans up to 5 levels deep. Specify a number to limit the depth:
...
For automated workflows, use `--yes` (or `-y`) to skip the confirmation prompt:
...
## Handling Complex FEL

Complex FEL files are skipped by default because converting them produces incorrect tone mapping. To override this safety check:
...
Use `--output` (or `-o`) to place converted files in a different directory:
...
Use `--temp` (or `-t`) to write temporary files to a separate, faster drive:
