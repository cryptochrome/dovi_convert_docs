...
Automatically delete backup files after each successful conversion:
...
<Warning>
This removes your original Profile 7 files permanently. There's no undo. Consider running a batch without `-delete` first, verify the results, then use `-cleanup` to remove backups.
</Warning>
