...
```
dovi_convert -convert <file> [options]
```

## Description

Converts a single Dolby Vision Profile 7 MKV file to Profile 8.1. The conversion strips the Enhancement Layer while injecting the RPU (dynamic metadata) into the Base Layer, creating a file compatible with devices that don't support FEL.
...
|----------|----------|-------------|
| `<file>` | Yes | Path to the Profile 7 MKV file to convert |
