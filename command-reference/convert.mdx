...
**`--yes` skips Simple FEL files by default.** For unattended operations, Simple FEL files are skipped unless you also pass `--include-simple`. This prevents accidental conversions of files that don't need it.
...
dovi_convert convert /movies -r 2
```
